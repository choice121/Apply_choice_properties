# Choice Properties Application System Ã¢ÂÂ Issue Tracker
  ...
  > Phase order matters Ã¢ÂÂ complete Phase A before starting Phase B, etc.
  ...
  **Remaining:** 2
  ...
  - `[ ]` Ã¢ÂÂ Open (not started)
  - `[~]` Ã¢ÂÂ In Progress
  - `[x]` Ã¢ÂÂ Fixed and verified
  - `[!]` Ã¢ÂÂ Blocked / needs decision

  ---

  ## Ã°ÂÂÂ´ CRITICAL ISSUES (C-series)

  ### [C1] Duplicate `Security Deposit` Case in `processApplication()`
  - **Status:** `[x]`
  - **File:** `backend/code.gs`
  - **Lines:** ~1179 and ~1205
  - **Severity:** CRITICAL Ã¢ÂÂ data loss risk
  ...
  - **Status:** `[x]`
  - **File:** `backend/code.gs`
  - **Lines:** `processApplication()` switch block (~1170Ã¢ÂÂ1220)
  - **Severity:** CRITICAL Ã¢ÂÂ silent data loss on schema change
  ...
  - **Note:** Can defer this rename if it's too disruptive Ã¢ÂÂ lower priority than C1 and C2.
  - **Test:** Submit a test app, verify the new column has the address and the label makes sense.

  ---

  ## Ã°ÂÂÂ  IMPORTANT ISSUES (I-series)

  ### [I1] No Validation That Property ID Exists in the Listing Platform
  - **Status:** `[x]`
  - **File:** `backend/code.gs`
  - **Function:** `processApplication()`
  - **Severity:** IMPORTANT Ã¢ÂÂ applications for non-existent/rented properties get through
  ...
  - **Severity:** IMPORTANT Ã¢ÂÂ API quota abuse risk
  ...
  - **Status:** `[x]` â Fixed April 8, 2026
  - **Files:** `js/script.js` (`setupSaveResume()`), `backend/code.gs` (`sendResumeEmail()`)
  - **Severity:** IMPORTANT Ã¢ÂÂ misleading UX
  ...
  - **Fix:** Update the email template in `sendResumeEmail()` to add a clear note:
    ```
    Ã¢ÂÂ Ã¯Â¸Â Important: This link must be opened in the same browser on the same device where you started your application.
    ```
  ...
  - **Status:** `[x]` â Fixed April 8, 2026
  - **File:** `js/script.js`
  - **Severity:** IMPORTANT Ã¢ÂÂ potential submission timeouts
  - **Problem:** `this.config.MAX_FILE_SIZE` is 10MB but `setupFileUploads()` enforces 4MB. Base64 encoding inflates 4 files ÃÂ 4MB = ~21MB total payload, which is within GAS limits but can cause timeout failures on slow connections.
  - **Fix:**
  ...
  - **Status:** `[x]`
  - **File:** `backend/code.gs`, `processApplication()` (~line 1108Ã¢ÂÂ1125)
  - **Severity:** IMPORTANT Ã¢ÂÂ duplicate applications can be submitted
  ...
  - **Fix:** Modify the duplicate check to use Property ID as the primary key when available:
    ...
    ```
  - **Test:** Submit an application. Try to submit again for the same property. Should be blocked. Try slightly modifying the address Ã¢ÂÂ should still be blocked (because Property ID matches).
  ...
  - **Status:** `[x]` â Fixed April 8, 2026
  - **File:** `_headers`
  - **Severity:** LOW-MEDIUM Ã¢ÂÂ future feature blocker
  ...
  ## Ã°ÂÂÂ¡ MODERATE ISSUES (M-series)
  ...
  - **Severity:** MODERATE Ã¢ÂÂ bad UX
  - **Problem:** When a user lands directly without URL params, `_showNoContextPrompt()` is called but the message and guidance are unclear Ã¢ÂÂ no link back to the listings page.
  ...
  - **Severity:** MODERATE Ã¢ÂÂ security risk
  ...
  - **Fix:** Reduce TTL to 8 hours (`8 * 60 * 60 * 1000` ms). Change in the inline script inside `doGet()`'s landing HTML:
    ...
    ```
    Also update the corresponding save logic if it exists. 30 Ã¢ÂÂ 8 hours.
  ...
  - **Severity:** MODERATE Ã¢ÂÂ compliance risk
  ...
  - **Status:** `[x]` â Fixed April 8, 2026
  - **File:** `js/script.js`, `backend/code.gs`
  - **Severity:** MODERATE Ã¢ÂÂ spam/abuse risk
  ...
  - **Severity:** IMPORTANT Ã¢ÂÂ listing platform gets stuck showing rented
  - **Problem:** `_syncPropertyStatusToSupabase(propertyId, 'rented')` is called when status Ã¢ÂÂ `approved`. But when status changes from `approved` to `withdrawn` or back to `denied`, the property stays `rented` in Supabase indefinitely.
  - **Fix:** In `updateStatus()`, add reverse sync logic:
    ```javascript
    // After the existing forward sync (approved Ã¢ÂÂ rented):
  ...
  - **Status:** `[x]`
  - **File:** `js/script.js`
  - **Severity:** MODERATE Ã¢ÂÂ makes debugging impossible in production
  ...
  ## Ã°ÂÂÂµ INTEGRATION ISSUES (B-series)

  ### [B1] GAS Backend URL Hardcoded in Two Separate Repos
  - **Status:** `[x]` â Fixed April 8, 2026
  - **Files:** `js/script.js` (this repo, line 74), `js/cp-api.js` (Choice repo Ã¢ÂÂ `CONFIG.APPLY_FORM_URL`), `admin/applications.html` (Choice repo)
  - **Severity:** IMPORTANT Ã¢ÂÂ maintenance hazard
  ...
  - **Severity:** MODERATE Ã¢ÂÂ edge case failure
  ...
  - **Severity:** MODERATE Ã¢ÂÂ no proactive failure detection
  ...
  - **Fix (GAS):** Add a health route to `doGet()`:
    ...
    ```
  - **Fix (Apply form):** Add a check in `handleFormSubmit()` before submission Ã¢ÂÂ if the health endpoint returns an error, show a user-friendly message instead of letting the full submission fail after a long wait.
  - **Fix (Listing platform Ã¢ÂÂ Choice repo):** Optionally, check the health endpoint before the Apply button redirects the user.
  ...
  |------|-------|----------|-------|
  | Ã¢ÂÂ | Ã¢ÂÂ | Ã¢ÂÂ | No fixes applied yet |
