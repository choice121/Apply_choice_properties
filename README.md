...
| Session | Changes |
|---------|---------| 
| 041 | **Phase 1 bug fixes — 2 issues resolved across `index.html` and `js/script.js`.** Full cross-repo analysis of `choice121/Apply_choice_properties` and `choice121/Choice` confirmed 23-param URL integration is fully aligned with zero mismatches. (1) **Critical — `hiddenPropertyAddress` missing `name` attribute:** `index.html` line 272 had `<input type="hidden" id="hiddenPropertyAddress">` with no `name`, silently dropping the URL-provided street address from every FormData submission. Fixed by adding `name="Property Address URL"`. (2) **Move-in date not enforced from `avail` param:** `_prefillFromURL()` stored the available date in `hiddenAvailableDate` but never applied it as a `min` attribute on `#requestedMoveIn`. Users could select move-in dates before the property was available. Fixed by setting `moveInField.min = avail` immediately after the hidden input is populated. |
